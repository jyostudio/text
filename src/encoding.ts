...
import List, { ReadOnlyList } from "@jyostudio/list";
...
export default abstract class Encoding {
    ...
    public static get ascii(): Encoding {
        return this.getEncoding("us-ascii");
    }
    ...
    public static get utf32(): Encoding {
        return this.getEncoding("utf-32");
    }
    ...
    public static get unicode(): Encoding {
        return this.getEncoding("utf-16-le");
    }
    ...
    public static get bigEndianUnicode(): Encoding {
        return this.getEncoding("utf-16-be");
    }
    ...
    public static convert(...params: any): any {
        Encoding.convert = overload()
            .add([Encoding, Encoding, Uint8Array], function (this: typeof Encoding, srcEncoding: Encoding, dstEncoding: Encoding, bytes: Uint8Array): Uint8Array {
                return Encoding.convert(srcEncoding, dstEncoding, bytes, 0, bytes.byteLength);
            })
            .add([Encoding, Encoding, Uint8Array, Number, Number], function (this: typeof Encoding, srcEncoding: Encoding, dstEncoding: Encoding, bytes: Uint8Array, index: number, count: number): Uint8Array {
        ...
        Encoding.getEncoding = overload()
            .add([Number], function (this: typeof Encoding, codePage: number): Encoding {
                ...
                for (let i = 0; i < this.#ENCODINGS.length; i++) {
                    if (this.#ENCODINGS[i].codePage === codePage) {
                        return this.#ENCODINGS[i].getEncoding();
                    }
                }

                throw new Error("未找到编码。");
            })
            .add([String], function (this: typeof Encoding, name: string): Encoding {
                for (let i = 0; i < this.#ENCODINGS.length; i++) {
                    const _name = name.toLowerCase();
                    for (let n = 0; n < this.#ENCODINGS[i].names.length; n++) {
                        if (this.#ENCODINGS[i].names[n] === _name) {
                            return this.#ENCODINGS[i].getEncoding();
                        }
                    }
                }
     ...
     */
    public abstract getByteCount(s: string): number;
     ...
     */
    public abstract getBytes(s: string): Uint8Array;
     ...
     */
    public abstract getBytes(s: string, index: number, count: number): Uint8Array;
     ...
     */
    public abstract getCharCount(bytes: Uint8Array): number;
     ...
     */
    public abstract getCharCount(bytes: Uint8Array, index: number, count: number): number;
    ...
    public getPreamble(...params: any): any {
        ...
        return Encoding.prototype.getPreamble.apply(this, params);
    }
     ...
     */
    public abstract getString(bytes: Uint8Array): string;
     ...
     * @throws {RangeError} index 或 count 不可小于零。 - 或 - index 和 count 不表示 bytes 中的有效范围。
     */
    public abstract getString(bytes: Uint8Array, index: number, count: number): string;
}